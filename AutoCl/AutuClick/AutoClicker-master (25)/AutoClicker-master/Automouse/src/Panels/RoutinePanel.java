...
public class RoutinePanel extends PanelSetting{
	...
	private int BoundY=1, DataNum, DataSize;
	...
	public void View(PlayEvent play) { //FramePanel로 인해 항상 화면 갱신
		...
		DataNum = (Constant.Page-1)*9; //데이타에 숫자를 정함. 버튼을 눌렀을때 그 데이터를 설정창으로 보내기 위함.
		
		if(play.isAlive()) Constant.Page = (play.getJ()/9)+1;
		...
		pages = new NomalLabel(Integer.toString(Constant.Page), Constant.PreferSize(250, 119, this.getWidth()),Constant.PreferSize(350, 319, this.getHeight()),Constant.PreferSize(250, 20, this.getWidth()),Constant.PreferSize(350, 20, this.getHeight())); //페이지 숫자표시 label
		DataSize = Constant.data.size(); //현재 Routine 크기를 저장

		for(int i=(Constant.Page-1)*9; i<DataSize && i<Constant.Page*9; i++) { //(페이지-1)*9 부터 데이터를 불러와서 9개까지 화면에 표시
	...
	class myActionListener implements ActionListener{
		...
		public void actionPerformed(ActionEvent e) {
			...
			try {
				...
				case "Prev" : //이전페이지
					if(Constant.Page != 1) { Constant.Page--; View(Play);}
					break;
				case "Next" : //다음페이지
					if(Constant.Page < (DataSize/9)+1) { Constant.Page++; View(Play);}
					break;
