...
public class ChucNangSV extends GuiSV
{
	...
	void GhiFile(SinhVien SVW)
	{
		try
		{
			FileOutputStream fos=new FileOutputStream("QLSV.db",true);
		...
		{
			JOptionPane.showMessageDialog(null,"Thêm Dữ Liệ Thành Công ");
		}
	}
	void DocFile()
	{
		try
		{
			FileReader fr=new FileReader("QLSV.db");
	...
	public void SoBangGhi()
	{
		...
		{
			int count=1;
			FileReader frd=new FileReader("QLSV.db");
	...
	public void Sua()
	{
		...
		try
		{
			FileOutputStream fos=new FileOutputStream("QLSV.db");
			PrintWriter pw;
			for(int i=0;i<SV.getSLSV();i++)
			{
				fos=new FileOutputStream("QLSV.db",true);
	...
	public void actionPerformed(ActionEvent e)
	{
		if(e.getSource()==bntThem)
		{
			...
			else
			{
				if(bntThem.getText().equalsIgnoreCase("Xóa"))
				{
					...
					{
						if(JOptionPane.showConfirmDialog(null,"Bạn Muốn Xóa Thông Tin Này ?","Xác Nhận",JOptionPane.YES_NO_OPTION,JOptionPane.QUESTION_MESSAGE)==JOptionPane.YES_OPTION)
                                                {
                                                    try
                                                    {
                                                         Del();
		...
		if(e.getSource()==bntLuu)
		{
			try
			{
				...
				{
					if(JOptionPane.showConfirmDialog(null,"Bạn Có Chắc Là Muốn Sửa Thông Tin Này Không ?","Xác Nhận",JOptionPane.YES_NO_OPTION,JOptionPane.QUESTION_MESSAGE)==JOptionPane.YES_OPTION)
					{
						Sua();
					}
				}
